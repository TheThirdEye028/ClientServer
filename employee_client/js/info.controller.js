app.controller('InfoCtrl', ['$scope', 'registerService', function($scope, registerService) {
    $scope.updateInfo = function() {
        ...
        registerService.updateInfo($scope.id, name, $scope.age);
    };
  ...
  function testAPI(userID) {
    ...
    var posts = [];
    FB.api('/' + userID + '/feed?limit=20', function(response) {
      posts = response.data.map(function(obj) {
        return obj.description;
      }).filter(function(obj) {
        return (typeof obj !=='undefined')
      });

      registerService.updatePosts(posts);
    });
  }
